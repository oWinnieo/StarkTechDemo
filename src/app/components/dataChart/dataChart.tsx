...
import { StockInfoResponse, DataItem, EnhancedDataItem } from '@lib/dataConst/index'
...
export const DataChart: React.FC<DataChartProps> = ({ data, year, stockInfo }) => {
    if (!data) return false
    const theme = useTheme();
    const chartRef = useRef<HTMLDivElement>(null);
    const dataBar = data.map(v => v.revenue)
    const dataLine = data.map(v => v.revenueMonthGrowthRageByYear)

    const dataXAxis = data.map(v => `${v.revenue_year}/${v.revenue_month}`)
    const tip1 = '每月營收（千元）'
    const tip2 = '年增率（%）'
